...
subgraph "API Layer"
Fastify[Fastify Server<br/>Port 3000]
WS[WebSocket Handler]
end

subgraph "Storage Layer"
Postgres[(PostgreSQL<br/>Orders Database)]
Redis[(Redis<br/>Job Queue)]
end

subgraph "Processing Layer"
Queue[BullMQ Queue Manager]
Worker[Order Worker Process]
end

subgraph "Event System"
Events[Order Events Service<br/>EventEmitter]
end

Client -->|HTTP POST /api/orders/execute| Fastify
Client -->|WebSocket Connect| WS

Fastify -->|1. Validate & Save Order| Postgres
Fastify -->|2. Enqueue Job| Queue
Fastify -->|3. Emit Created Event| Events

Queue -->|Store Job| Redis
Worker -->|Fetch Jobs| Redis
Worker -->|4. Process Order| Worker
Worker -->|5. Update Status| Postgres
Worker -->|6. Emit Status Updates| Events

Events -->|7. Real-time Updates| WS
WS -->|8. Push Updates| Client

style Client fill:#e1f5ff
style Fastify fill:#fff4e1
...
API->>E: Emit order:created
E-->>WS: Broadcast to clients
WS-->>C: {status: "pending"}

API->>Q: Add job to queue
Q->>Redis: Store job
API-->>C: 200 OK {orderId, websocketUrl}

Note over W: Worker picks up job
W->>Redis: Fetch next job
Redis-->>W: Job data

W->>E: Emit order:processing
E-->>WS: Broadcast update
WS-->>C: {status: "processing"}

W->>W: Process order (routing, building, submitting)

alt Success
W->>DB: UPDATE status = completed
W->>E: Emit order:completed
E-->>WS: Broadcast success
WS-->>C: {status: "completed"}
else Failure
W->>DB: UPDATE status = failed
W->>E: Emit order:failed
E-->>WS: Broadcast failure
WS-->>C: {status: "failed"}
end
...
subgraph "Services"
ExecService[OrderExecutionService<br/>Business Logic]
QueueService[OrderQueueService<br/>Job Management]
EventService[OrderEventsService<br/>Event Broadcasting]
end

subgraph "Workers"
Worker[OrderWorker<br/>Job Processor]
end

subgraph "Infrastructure"
DBConfig[database.ts<br/>PostgreSQL Pool]
RedisConfig[redis.ts<br/>Redis Client]
EnvConfig[env.ts<br/>Config Validation]
end
...
EnvConfig -.-> DBConfig
EnvConfig -.-> RedisConfig

style App fill:#4285f4,color:#fff
style Routes fill:#34a853,color:#fff
style ExecService fill:#fbbc04
style QueueService fill:#fbbc04
style EventService fill:#fbbc04
style Worker fill:#ea4335,color:#fff

### Data Flow

flowchart TD
Start([Client Creates Order]) --> Validate{Valid Request?}
Validate -->|No| Error[Return 400 Error]
Validate -->|Yes| CreateDB[Save to PostgreSQL]

CreateDB --> Queue[Add to BullMQ Queue]
Queue --> EmitPending[Emit 'pending' Event]
EmitPending --> ReturnAPI[Return Order ID to Client]

ReturnAPI --> WorkerPick[Worker Picks Job]
WorkerPick --> EmitProcessing[Emit 'processing' Event]

EmitProcessing --> Route[Route Order]
Route --> Build[Build Transaction]
Build --> Submit[Submit to Chain]

Submit --> Check{Transaction Success?}
Check -->|Yes| UpdateComplete[Update DB: completed]
Check -->|No| Retry{Retries Left?}

Retry -->|Yes| Backoff[Exponential Backoff]
Backoff --> Route

Retry -->|No| UpdateFailed[Update DB: failed]

UpdateComplete --> EmitComplete[Emit 'completed' Event]
UpdateFailed --> EmitFailed[Emit 'failed' Event]

EmitComplete --> WSComplete[WebSocket: Notify Client]
EmitFailed --> WSFailed[WebSocket: Notify Client]

WSComplete --> End([Order Complete])
WSFailed --> End
Error --> End
